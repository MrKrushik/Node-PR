...
module.exports.fetchAllusers = async (req,res) => {
    try{
        ...
        const allUsers = await userAuth.fetchAllUsers();

        if (!allUsers) {
            return res.status(500).json(errorResponse(500,false,MSG.USER_UNAUTHORIZED))
        }

        return res.status(200).json(successResponse(200,false, MSG.ALL_USER_FETCHED_SUCCESSFULLY, {allUsers}))
    }catch(err){
        console.log("error := ",err);
